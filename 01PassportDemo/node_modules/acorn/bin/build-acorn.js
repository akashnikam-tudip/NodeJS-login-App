...
browserify({standalone: "acorn"})
  ...
  .transform(babelify)
  .require("./src/index.hbs", {entry: true})
  .bundle()
...
function acornShimPrepare(file) {
  ...
  if (file == path.resolve(__dirname, "../src/index.hbs")) {
    var sent = false
...
browserify({standalone: "acorn.loose"})
  ...
  .transform(babelify)
  .require("./src/loose/index.hbs", {entry: true})
  .bundle()
  .on("error", function (err) { console.log("Error: " + err.message) })
...
browserify({standalone: "acorn.walk"})
  ...
  .transform(acornShimPrepare)
  .transform(babelify)
  .require("./src/walk/index.hbs", {entry: true})
  .bundle()
  .on("error", function (err) { console.log("Error: " + err.message) })
  .pipe(acornShimComplete())
